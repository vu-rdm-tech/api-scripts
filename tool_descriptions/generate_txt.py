...
def write_to_file(idata, filename):
    ...
    with open(idata, 'r') as F:
        ...
        cntr = 1
        for row in data:
            idx += ' <tr><td align="center">{}</td><td align="left"><a href="#tool_{}">{}</a>.</td>'.format(
                cntr, row['Name'].replace(' ', ''), row['Name']
            )
        ...
        for row in data:
            ...
            body += '<h3><a id="tool_{}">{}</a> (<a href="{}">{}</a>)</h3>\n'.format(
                row['Name'].replace(' ', ''), row['Name'], row['URI'], display_uri
            )
            keyword = row['Keyword']
            if keyword != '':
            ...
            if row['Description'] == 'Empty':
                ...
                if len(display_uri.split('.')) > 2:
                    src_uri = display_uri.split('.', len(display_uri.split('.')) - 2)[
                        -1
                    ]
                else:
                    src_uri = display_uri
